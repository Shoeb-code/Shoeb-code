# Hi there, I'm Shoeb Khan 👋

### 🚀 Full-Stack Developer (MERN) | Backend-Focused | Problem Solver

I’m a **Full-Stack Developer specializing in MERN Stack development**, with hands-on experience building **scalable RESTful APIs, secure authentication systems, and responsive React applications**.

💻 Backend-focused developer with strong expertise in **Node.js, Express.js, MongoDB, and JWT Authentication**
⚡ Passionate about **clean architecture, scalable systems, and performance optimization**
📚 Strong foundation in **Data Structures & Algorithms (420+ LeetCode problems solved)**
🎯 Actively seeking **Software Development opportunities**

---

## 🛠 Tech Stack

### Languages

![JavaScript](https://img.shields.io/badge/JavaScript-000?logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-000?logo=TypeScript)
![Java](https://img.shields.io/badge/Java-000?logo=java)
![HTML](https://img.shields.io/badge/HTML-000?logo=html5)
![CSS](https://img.shields.io/badge/CSS-000?logo=css3)

### Frontend

![React](https://img.shields.io/badge/React-000?logo=react)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-000?logo=tailwindcss)
![Bootstrap](https://img.shields.io/badge/Bootstrap-000?logo=bootstrap)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-000?logo=node.js)
![Express.js](https://img.shields.io/badge/Express.js-000?logo=express)

### Database

![MongoDB](https://img.shields.io/badge/MongoDB-000?logo=mongodb)
![MySQL](https://img.shields.io/badge/MySQL-000?logo=mysql)

### Tools

![Git](https://img.shields.io/badge/Git-000?logo=git)
![GitHub](https://img.shields.io/badge/GitHub-000?logo=github)
![Postman](https://img.shields.io/badge/Postman-000?logo=postman)
![Docker](https://img.shields.io/badge/Docker-000?logo=docker)
![VS Code](https://img.shields.io/badge/VS_Code-000?logo=visualstudiocode)

---

## 💼 Experience

### Backend Developer — OnNextWeb

* Built scalable RESTful APIs for authentication and user management
* Implemented JWT authentication and RBAC
* Optimized MongoDB queries for better performance
* Integrated Cloudinary media upload pipeline

### Frontend Developer — Codefast Platform

* Built reusable React components
* Integrated frontend with backend APIs
* Improved UI consistency and user experience

---

## 🚀 Featured Project

### FastTutors — Full-Stack Tutor/student connect  Platform

A full-stack tutoring platform connecting students with verified tutors.

**Tech Stack:** React.js, Node.js, Express.js, MongoDB

### Key Features

* JWT Authentication
* Role-Based Access Control
* Tutor Search & Filtering
* Optimized MongoDB Indexing
* Admin Dashboard

---

## 🔥 Current Focus

* Backend Development
* Scalable REST APIs
* Authentication Systems
* MERN Stack Projects
* DSA & System Design

---

## 🤝 Let's Connect

Open to **Software Development roles and freelance projects**
